 ...
 * @param {string} npmRegistry the registry to replace for
 * @returns {string} the resolved line with the INTERNAL_REGISTRY flag
 */
function replaceHost(resolvedField, npmRegistry) {
  const registry = npmRegistry.charAt(npmRegistry.length - 1) !== '/' ? `${npmRegistry}/` : npmRegistry;
  return resolvedField.replace(REG_EX, `${registry}$2`);
}

...
function treatLine(line, registry) {
  if (line.startsWith('  resolved "http')) {
    return commonLock.replaceHost(line, registry).concat('\n');
  }
 ...
 */
function run(folderPath, outputFolder, registry) {
  ...
  if (fs.existsSync(filePath, 'utf8')) {
    ...
    readline.eachLine(filePath, (line, last) => {
      content = content.concat(treatLine(line, registry));
      if (last) {

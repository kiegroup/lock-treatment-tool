...
function treatFields(dependencies, npmOptions = new NpmOptions()) {
  if (npmOptions.replacePackageLockRegistry === true) {
    if (npmOptions.registry === undefined) {
      throw Error('replacePackageLockRegistry flag defined but registry');
    }
  }
  Object.keys(dependencies).forEach((prop) => {
    const element = dependencies[prop];
    if (npmOptions.replacePackageLockRegistry && npmOptions.npmRegistry) {
      element.resolved = commonLock.replaceHost(element.resolved, npmOptions.npmRegistry);
    } else {
      delete element.resolved;
    }
    if (npmOptions.skipIntegrity !== true) {
      delete element.integrity;
    }
    if (element.dependencies) {
      treatFields(element.dependencies);
    }
  });
}
...
function run(folderPath, outputFolder, npmOptions) {
  return treatLockFields(folderPath, outputFolder, PACKAGE_LOCK_FILE_NAME, npmOptions)
    || treatLockFields(folderPath, outputFolder, SHRINKWRAP_FILE_NAME, npmOptions);
}

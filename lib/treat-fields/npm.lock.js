...
function run(folderPath, outputFolder) {
    _removeLockFields(folderPath, outputFolder, PACKAGE_LOCK_FILE_NAME);
    _removeLockFields(folderPath, outputFolder, SHRINKWRAP_FILE_NAME);
}
 ...
 */
function _removeLockFields(folderPath, outputFolder, fileName) {
    const fs = require('fs')

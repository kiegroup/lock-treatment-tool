 ...
 */
function replaceHost(line, npmRegistry) {
  return line.replace(/(https?:\/\/[^\s$#^@]*\/)((@[^\s/$]*\/)?[^\s/$]*\/-\/.*)/, `${npmRegistry}$2`);
}

function treatLine(line, npmRegistry) {
  if (line.startsWith('  resolved "http')) {
    return replaceHost(line, npmRegistry).concat('\n');
  }
  ...
  fs.readFile(filePath, 'utf8', (readErr) => {
    if (readErr) {
      ...
      readline.eachLine(filePath, (line, last) => {
        content = content.concat(treatLine(line, npmRegistry));
        if (last) {

...
test('Verify it works when the file package-lock.json exists', () => {
  const uuid = uuidv4();
  // Act
  expect(npmLock('./test/resources/npmv9', `./test/resources/npmv9/execution-${uuid}`)).toBe(true);
});
...
test('Verify package-lock.json', () => {
  ...
  // Act
  npmLock('./test/resources/npmv9', `./test/resources/npmv9/execution-${uuid}`);

  // Assert
  const json = JSON.parse(fs.readFileSync(`./test/resources/npmv9/execution-${uuid}/package-lock.json`, 'utf8'));
...
test('Verify registry replacement', () => {
  ...
  // Act
  npmLock('./test/resources/npmv9', `./test/resources/npmv9/execution-${uuid}`, npmOptions);
